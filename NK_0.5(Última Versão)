...
def listar_processos():
    ...
    processos_ordenados = sorted(
        ...
        key=ordenar_processos
    )

    atualizar_treeview(processos_ordenados)
    janela.after(5000, listar_processos)

def ordenar_processos(item):
    global ordenacao_atual
    if ordenacao_atual == "Categoria":
        return (ordem_categorias.index(item[1]), -item[2], item[0].lower())
    elif ordenacao_atual == "Nome do Processo":
        return (item[0].lower())
    elif ordenacao_atual == "Quantidade de Tarefas":
        return (-item[2], item[0].lower())
...
def exibir_ajuda():
    ajuda_texto = (
        ...
        "5. Ao lado é possível selecionar se as tarefas serão exibidas por: Nome da Tarefa, Nome da Categoria ou Por Quantidade de Tarefas.\n\n"
...
dropdown_ordenacao = ttk.Combobox(frame_botoes, values=["Categoria", "Nome do Processo", "Quantidade de Tarefas"], state="readonly")
dropdown_ordenacao.set("Categoria")
dropdown_ordenacao.bind("<<ComboboxSelected>>", alterar_ordenacao)
dropdown_ordenacao.pack(side=tk.LEFT, padx=10)
